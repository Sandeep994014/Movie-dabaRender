...
const Main = () => {
  const handleCardClick = (movie) => {
    const url = `/movieDetails?title=${encodeURIComponent(movie.Title)}&year=${
      movie.Year
    }&imdbID=${movie.imdbID}&poster=${encodeURIComponent(movie.Poster)}`;
    window.open(url); 
  };
    ...
    <div className="p-5">
      <Row>
        {data.map((movie, index) => (
          <Col key={index} className="col-lg-2 col-6 pt-3">
